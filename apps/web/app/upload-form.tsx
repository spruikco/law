...
export function UploadForm() {
  ...
  const useSamples = async () => {
    setError(null);
    try {
      const samples = await fetchSamplePdfs(["contract-of-sale.pdf", "section-32.pdf"]);
      setFiles(samples);
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="block text-sm font-medium text-zinc-700">
              PDFs — Contract of Sale + Section 32 (plus any supporting docs)
            </label>
            <button
              type="button"
              onClick={useSamples}
          ...
          >
            <div className="text-sm text-zinc-600">
              Drag PDFs here or <span className="text-zinc-900 font-medium">click to browse</span>
            </div>
            <input
              id="file-input"
              type="file"
