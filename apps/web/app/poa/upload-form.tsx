...
type Role = "principal" | "attorney" | "interested_party";

export function PoaUploadForm() {
  const router = useRouter();
  const [files, setFiles] = useState<File[]>([]);
  const [clientName, setClientName] = useState("");
  const [clientRole, setClientRole] = useState<Role>("principal");
  ...
  const useSamples = async () => {
    setError(null);
    try {
      const samples = await fetchSamplePdfs(["poa.pdf"]);
      setFiles(samples);
      setClientName("Margaret Wilson");
      setClientRole("principal");
    } catch (err) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/poa-review", {
        method: "POST",
        body: form,
      });
      if (!res.ok) throw new Error(await res.text());
      const { id } = (await res.json()) as { id: string };
      router.push(`/poa-review/${id}`);
    } catch (err) {
      setError(err instanceof Error ? err.message : String(err));
      setSubmitting(false);
    }
  ...
  return (
    <div className="rounded-xl border border-zinc-200 bg-white p-8 shadow-sm">
      <div className="space-y-6">
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          ...
          <div>
            ...
            <select
              ...
              disabled={submitting}
            >
              <option value="principal">Principal (donor)</option>
              <option value="attorney">Attorney</option>
              <option value="interested_party">Interested party / family</option>
            </select>
          </div>
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <label className="block text-sm font-medium text-zinc-700">
              Power of Attorney PDF(s) — instrument, acceptance, witness certificates
            </label>
            <button
              type="button"
              onClick={useSamples}
        ...
        >
          {submitting ? "Starting review…" : "Start Power of Attorney review"}
        </button>
      </div>
    </div>
  );
